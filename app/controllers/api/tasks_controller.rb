class Api::TasksController < ApplicationController
  ...
  def search_check(param)
    if param.present?
      ...
      grouping_hash = key_words.reduce({}) do |hash, word|
        hash.merge(word => { content_or_assigner_screen_name_or_description_or_project_name_cont: word })
      end
    else
      nil
    end
  ...
  def index
    if params[:q].nil?
      ...
      @q = Task.joins(:state).ransack({ combinator: 'and',
                                        groupings: search_check(params[:q][:content_or_assigner_screen_name_or_description_or_project_name_cont]) })
  ...
  def create
    ...
    if @task.save!
      ...
      ActionItem.find(matched[1]).update(task_url: tasks_path + '/' + @task.id.to_s) unless matched.nil?
